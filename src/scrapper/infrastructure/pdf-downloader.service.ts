  ...
  async downloadAsBase64(pdfUrl: string): Promise<string | null> {
    try {
      ...
      if (!contentType.includes('pdf') && !contentType.includes('octet-stream')) {
        this.logger.warn(`Unexpected content-type "${contentType}" for ${pdfUrl}`);
      }

      const buffer = await response.arrayBuffer();
      const base64 = Buffer.from(buffer).toString('base64');

      this.logger.log(`PDF downloaded (${Math.round(buffer.byteLength / 1024)} KB)`);
      return base64;
    } catch (error) {
      this.logger.error(`Failed to download PDF: ${pdfUrl}`, (error as Error).message);
      return null;
    }
  }
}
