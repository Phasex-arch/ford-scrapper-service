...
export interface IPortDBDataVehicle {
    ...
    findDistinctCategories(): Promise<{ categoria_principal: string; count: number }[]>;
    findDistinctModels(): Promise<{ modelo: string; familia: string; count: number }[]>;
    findDistinctVersions(): Promise<{ versao: string; modelo: string; count: number }[]>;
    ...
    getAllSources(): Promise<{ modelo_url: string; versao_url: string | null; ficha_tecnica_url: string | null; cores_url: string }[]>;
